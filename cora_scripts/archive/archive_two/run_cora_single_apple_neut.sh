...
LOGFILE="logs/cora_single_apple_neut.log"
...
CUDA_VISIBLE_DEVICES=1 nohup python cora_src/main_cora_anc.py \
  ...
  --anchor_pool "a food item, a fruit" \
  --preserve_concepts "a Pineapple, a Custard apple" \
  ...
  --xformers --tf32 \
  --debug \
  --save_path "outputs_cora_anc/Apple/neut" \
