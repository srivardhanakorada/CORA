...
class VisualAttentionProcess(nn.Module):
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states, *args, **kwargs):
        ...
        attn.processor = AttnProcessor(
            ...
            self.record_type, self.sigmoid_setting, self.token_sim, self.decomp_timestep
        )
        ...
        if not self.record and encoder_hidden_states.shape[1] == 77 and self.atten_type != 'original':
            if 'queries' in self.target_records:
                erase_query, retain_query = self.record_ortho_decomp(
                    target_record=self.target_records['queries'], current_record=query
                )
            ...
            if 'keys' in self.target_records:
                erase_key, retain_key = self.record_ortho_decomp(
                    target_record=self.target_records['keys'], current_record=key
                )
        ...
        if not self.record and encoder_hidden_states.shape[1] == 77 and self.atten_type != 'original':
            if 'attn_maps' in self.target_records:
                erase_attention_probs, retain_attention_probs = self.record_ortho_decomp(
                    target_record=self.target_records['attn_maps'], current_record=attention_probs
                )
        ...
        if encoder_hidden_states.shape[1] != 77:
            ...
            if self.record:
                ...
                erase_value, retain_value = self.record_ortho_decomp(
                    target_record=self.target_records['values'], current_record=value
                )
...
def set_attenprocessor(unet, atten_type='original', target_records=None, record=False, record_type=None,
                       sigmoid_setting=None, token_sim=False, decomp_timestep=0):
...
def diffusion(unet, scheduler, latents, text_embeddings, total_timesteps, start_timesteps=0,
    ...
    for timestep in tqdm(scheduler.timesteps[start_timesteps: total_timesteps], desc=desc):
        ...
        if record:
            for type_ in record_type.strip().split(','):
                for value in unet.attn_processors.values():  # not the same 'value' as CA/SA value
                    for k, v in value.records[type_].items():
...
def make_latents_from_seeds(seeds, device, dtype):
    outs = []
    for s in seeds:
        gen = torch.Generator('cpu').manual_seed(int(s))
        outs.append(torch.randn((1, 4, 64, 64), generator=gen))
...
class AdaDataset(Dataset):
    def __init__(self, data_path, seed=None, guidance_scale=None, max_num=100000):
        ...
        self.idx = [int(x) for x in list(self.data['id'])[:max_num]]
        self.seed = [int(x) for x in list(self.data['seed'])[:max_num]]
...
def main():
    ...
    parser.add_argument('--total_timesteps', type=int, default=30, help='Total timesteps of sampling process')
    parser.add_argument('--num_samples', type=int, default=10, help='Number of samples per prompt to generate')
    parser.add_argument('--batch_size', type=int, default=10, help='Batch size of sampling process')
    ...
    parser.add_argument('--contents', type=str, default='erase, retention', help='contents to run: erase, retention')
    ...
    # Prepare embeddings for target concepts and uncond
    ...
    target_concept_encoding = torch.concat(target_concept_encoding) if len(target_concept_encoding) > 0 else None
    uncond_encoding = get_textencoding(get_token('', tokenizer), text_encoder)

    # Main loop over contents (erase/retention)
    for content in args.contents.split(', '):
        content = content.strip()
        if content not in ['erase', 'retention']:
            continue

        for count, data in enumerate(dataloader):
            # Select only items in this batch that match current content
            types = data['type'] if isinstance(data['type'], list) else [data['type']]
            sel = [i for i, t in enumerate(types) if t == content]
            if not sel:
                continue

            # Subset the batch
            sub_prompts = [data['prompt'][i] for i in sel]
            sub_ids = [int(data['idx'][i]) for i in sel]
            sub_seeds = [int(data['seed'][i]) for i in sel]
            n = len(sub_prompts)

            # Compute text embeddings for subset (once)
            text_emb = get_textencoding(get_token(sub_prompts, tokenizer), text_encoder)

            # Build similarity ordering for target concept encodings if provided
            if target_concept_encoding is not None:
                # cosine sim between each prompt encoding and each target concept
                embedding_sim = torch.cosine_similarity(
                    text_emb.unsqueeze(1), target_concept_encoding, dim=-1
                ).sum(-1).max(dim=-1)[-1].tolist()
                trans_target_encodings = trans_target(embedding_sim, copy.deepcopy(target_concept_encoding))

                # Record target records (values) once per subset
                unet_rec = set_attenprocessor(unet, atten_type='original', record=True, record_type='values')
            ...
            # Latents: one per seed
            latents = make_latents_from_seeds(sub_seeds, pipe.device, pipe.dtype)
            ...
            if 'original' in mode_list:
                ...
                save_images['original'] = diffusion(
                    ...
                    latents=latents, start_timesteps=0,
                    text_embeddings=torch.cat([uncond_encoding] * n + [text_emb], dim=0),
                    total_timesteps=args.total_timesteps,
                    guidance_scale=args.guidance_scale,
                    desc=f"{count} x prompts | original"
                )
                del unet_original
            ...
            if 'retain' in mode_list:
                ...
                unet_retain = set_attenprocessor(
                    ...
                    sigmoid_setting=(100, 0.9, 2), token_sim=True
                )
                save_images['retain'] = diffusion(
                    ...
                    total_timesteps=args.total_timesteps,
                    guidance_scale=args.guidance_scale,
                    desc=f"{count} x prompts | retain"
                )
                del unet_retain

            # Save paths per content (erase/retention)
            ...
            # Save one file per subset item (no mixing)
            for j in range(n):
                prompt_j = sub_prompts[j]
                id_j = sub_ids[j]
                save_filename = re.sub(r'[^\w\s]', '', prompt_j).replace(' ', '_') + f'_{id_j}.png'

                imgs_to_combine = []
                for mode in mode_list:
                    decoded_imgs[mode][j].save(os.path.join(save_path, mode, save_filename))
                    imgs_to_combine.append(decoded_imgs[mode][j])

                if len(mode_list) > 1:
                    total_w = sum(im.size[0] for im in imgs_to_combine)
                    max_h = max(im.size[1] for im in imgs_to_combine)
                    img_combined = Image.new('RGB', (total_w, max_h))
                    x = 0
                    for im in imgs_to_combine:
                        img_combined.paste(im, (x, 0))
                        x += im.size[0]
