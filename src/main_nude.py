...
class AttnProcessor():
    ...
    def cal_ortho_decomp(self, target_value, pro_record, ortho_basis=None, project_matrix=None): 
        ...
        if ortho_basis is None and project_matrix is None:
            ...
            tar_record_ = rearrange(target_value, 'b h l d -> l b (h d)') # [num_concepts, 77, 640]
    ...
    def cal_gram_schmidt(self, target_value):
        # Reshape and permute `target_value` as required
        target_value = target_value.view((2, len(target_value) // 16, -1) + target_value.size()[-2:])
        target_value = target_value.permute(1, 0, 2, 3, 4).contiguous().view((target_value.size(1), -1) + target_value.size()[-2:])
        target_value = rearrange(target_value, 'b h l d -> b l (h d)')
        
        # Initialize result tensors
        project_matrix = torch.zeros((target_value.size(1), 2, 2), dtype=target_value.dtype, device=target_value.device)
        basis_ortho = torch.zeros((target_value.size(1), 2, target_value.size(-1)), dtype=target_value.dtype, device=target_value.device)
        
        # Gram-Schmidt orthogonalization for each vector in target_value
        for i in range(target_value.size(1)):
            V = target_value[:, i, :]
            n = V.size(0)
            proj_matrix = torch.eye(n, dtype=V.dtype, device=V.device)
            for j in range(1, n):
                for k in range(j):
                    proj_matrix[j, k] = - torch.dot(V[k], V[j]) / torch.dot(V[k], V[k])
            ortho_basis = proj_matrix @ V  # Compute orthogonal basis directly
            project_matrix[i] = proj_matrix  # Store project matrix
            basis_ortho[i] = ortho_basis  # Store orthogonal basis
...
def main():
    ...
    parser.add_argument('--num_samples', type=int, default=1, help='The number of samples per prompt to generate' )
    parser.add_argument('--batch_size', type=int, default=1, help='The batch size of the sampling process')
    # Erasing Config
    parser.add_argument('--erase_type', type=str, default='', help='instance, nsfw, style')
    parser.add_argument('--target_concept', type=str, default='')
    parser.add_argument('--contents', type=str, default='nudity, coco')
    parser.add_argument('--token_sim', default=False, action='store_true', help='Whether to use token similarity')
    parser.add_argument('--sigmoid_a', type=float, default=10.0)
    parser.add_argument('--sigmoid_b', type=float, default=0.45)
    ...
    for data in dataloader:
        latent = torch.randn((bs, 4, 64, 64), generator=torch.Generator('cpu').manual_seed(data['seed'][0].item())).to(pipe.device, dtype=target_concept_encoding.dtype)
        ORTHO_DECOMP_STORAGE, Images = {}, {}

        encoding = get_textencoding(get_token(data["prompt"], tokenizer), text_encoder)
        ...
        if 'original' in mode_list:
            Images['original'] = diffusion(unet=unet_original, scheduler=pipe.scheduler, 
                                        ...
                                        text_embeddings=torch.cat([uncond_encoding] * bs + [encoding], dim=0), 
                                        total_timesteps=args.total_timesteps, 
                                        guidance_scale=data['guidance'][0].item(), 
                                        desc=f"{data['prompt'][0]} | original")
        ...
        if 'retain' in mode_list:
            ...
            Images['retain'] = diffusion(unet=unet_retain, scheduler=pipe.scheduler, 
                                    ...
                                    total_timesteps=args.total_timesteps, 
                                    guidance_scale=data['guidance'][0].item(), 
                                    desc=f"{data['prompt'][0]} | retain")
            
        save_path = os.path.join(args.save_root, args.target_concept.replace(', ', '_'), args.contents)
        ...
        for idx in range(len(decoded_imgs[mode_list[0]])):
            save_filename = f'{data["prompt_idx"][idx]}_'+re.sub(r'[^\w\s]', '', data["prompt"][idx]).replace(' ', '_')[:30]+ f"_{int(idx + bs * idx)}.png"
            images_to_combine = []
