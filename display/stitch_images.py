...
#                 f"Looked for: {os.path.join(image_dir, f'A_photo_of_{name}_*.png')} (and .PNG)\n"
#                 f"Found: {found}"
#             )
...
#!/usr/bin/env python3
"""
Two-row stitcher (1xN each):
For each matched file "A_photo_of_{name}_{idx}.png" (or .PNG), split vertically into
left and right halves, then build TWO single-row images:
  - LEFT row  -> --out-left  (concatenated left halves)
  - RIGHT row -> --out-right (concatenated right halves)

Behavior:
- If --names is provided, the script expects exactly ONE image per name (picks the smallest index if multiple).
- If --names is omitted, it uses ALL images in --image-dir (sorted by index) and concatenates them.

Each half is letterboxed independently into square tiles (size --cell), then pasted left→right with gap.
"""
...
# Accept 1–3 digit suffixes like _7.png, _099.png, _999.png
FNAME_RE = re.compile(r"_(\d{1,3})\.(png|PNG)$")
...
def load_names(args) -> List[str]:
    # Support: --names "A" "B" OR --names "A, B" OR a file via --names-file
    if args.names_file:
        with open(args.names_file, "r", encoding="utf-8") as f:
            names = [ln.strip() for ln in f if ln.strip()]
    else:
        ...
        if len(names) == 1 and ("," in names[0]):
            names = [n.strip() for n in names[0].split(",") if n.strip()]
    return names  # can be empty (meaning: use all images found)

def pat_variants(image_dir: str, name: str = None) -> List[str]:
    """Build glob patterns for space and underscore variants."""
    if name is None:
        # All images in dir, any prefix that matches A_photo_of_*_###.png
        return [
            os.path.join(image_dir, "A_photo_of_*_[0-9][0-9][0-9].png"),
            os.path.join(image_dir, "A_photo_of_*_[0-9][0-9][0-9].PNG"),
            os.path.join(image_dir, "A_photo_of_*_[0-9][0-9].png"),
            os.path.join(image_dir, "A_photo_of_*_[0-9][0-9].PNG"),
            os.path.join(image_dir, "A_photo_of_*_[0-9].png"),
            os.path.join(image_dir, "A_photo_of_*_[0-9].PNG"),
            os.path.join(image_dir, "A photo of *_*.png"),
            os.path.join(image_dir, "A photo of *_*.PNG"),
        ]
    # Name given: allow both "A photo of {name}" and "A_photo_of_{name}"
    name_space = name.strip()
    name_unders = "_".join(name.split())  # collapse whitespace to underscores
    return [
        os.path.join(image_dir, f"A photo of {name_space}_*.png"),
        os.path.join(image_dir, f"A photo of {name_space}_*.PNG"),
        os.path.join(image_dir, f"A_photo_of_{name_unders}_*.png"),
        os.path.join(image_dir, f"A_photo_of_{name_unders}_*.PNG"),
    ]
...
def pick_one_for_name(image_dir: str, name: str) -> str:
    """Return a single path for the given name (smallest index)."""
    cands = list_images(pat_variants(image_dir, name))
    cands = [(parse_index(p), p) for p in cands if parse_index(p) >= 0]
    if not cands:
        raise SystemExit(
            f'Expected one image for "{name}", found 0.\n'
            f"Looked for patterns like:\n  " +
            "\n  ".join(pat_variants(image_dir, name))
        )
    cands.sort(key=lambda x: x[0])
    return cands[0][1]  # smallest index

def collect_paths(image_dir: str, names: List[str]) -> List[str]:
    if names:
        # One image per provided name
        return [pick_one_for_name(image_dir, nm) for nm in names]
    # No names: take ALL images in dir (sorted by index then name)
    cands = list_images(pat_variants(image_dir, None))
    parsed = [(parse_index(p), p) for p in cands if parse_index(p) >= 0]
    if not parsed:
        raise SystemExit(f"No images found in {image_dir} matching expected patterns.")
...
def split_vertical(im: Image.Image):
    """Split image into left and right halves (right gets extra pixel if odd width)."""
    w, h = im.size
    mid = w // 2
    left_box = (0, 0, mid, h)
    right_box = (mid, 0, w, h)
...
def make_two_rows(paths: List[str], out_left: str, out_right: str,
                  cell: int, gap: int, bg):
    cols = len(paths)
    if cols == 0:
        raise SystemExit("No input images to process.")
    W = cols * cell + (cols - 1) * gap
    H = cell  # single row

    canvas_L = Image.new("RGB", (W, H), color=bg)
    canvas_R = Image.new("RGB", (W, H), color=bg)
    ...
    for c, fp in enumerate(paths):
        with Image.open(fp) as im:
            ...
            tile_R = letterbox(right_half, cell, bg=bg)
        x = c * (cell + gap)
        canvas_L.paste(tile_L, (x, 0))
        canvas_R.paste(tile_R, (x, 0))

    os.makedirs(os.path.dirname(out_left) or ".", exist_ok=True)
    os.makedirs(os.path.dirname(out_right) or ".", exist_ok=True)
    canvas_L.save(out_left)
    canvas_R.save(out_right)
    print(f"Saved LEFT row  (original halves) -> {out_left}")
    print(f"Saved RIGHT row (eraser   halves) -> {out_right}")
...
def main():
    ...
    ap.add_argument("--names", nargs="*", help='List of names, e.g. --names "Donald Trump" "Dog"')
    ap.add_argument("--names-file", help="Text file with one name per line")
    ap.add_argument("--out-left", default="row_left.png", help="Output PNG for LEFT halves row")
    ap.add_argument("--out-right", default="row_right.png", help="Output PNG for RIGHT halves row")
    ap.add_argument("--cell", type=int, default=384, help="Square tile size (default: 384)")
    ap.add_argument("--gap", type=int, default=8, help="Gap between tiles (default: 8)")
    ap.add_argument("--bg", default="#FFFFFF", help="Background color hex (default: #FFFFFF)")
    args = ap.parse_args()
    ...
    names = load_names(args)
    paths = collect_paths(args.image_dir, names)
    make_two_rows(paths, args.out_left, args.out_right, args.cell, args.gap, bg_rgb)
