...
#!/usr/bin/env python3
"""
CORA / AdaVD Evaluation Script
==============================

Compute:
  - ESR (Erase Success Rate) = 1 - mean(CLIP similarity(targets vs canonical))
  - PSR (Preserve Success Rate) = mean(CLIP similarity(non-targets vs own))
  - HM  (Harmonic Mean) = 2 / (1/ESR + 1/PSR), ESR/PSR clamped to [0,1] for HM

Matching rule:
  → Any image file whose *filename contains* a target/non-target concept
    (case-insensitive, ignoring underscores, hyphens) is included.

Usage example:
---------------
python metrics/src/cs_metrics.py \
  --folder results/cora/artist_vangogh/erase/ \
  --targets "Van Gogh" \
  --non_targets "Monet, Picasso, Andy Warhol, Caravaggio" \
  --csv_out metrics_artist_vangogh.csv
...
def normalize_simple(s: str) -> str:
    """Lowercase, replace underscores/hyphens with spaces, strip punctuation."""
    return s.lower().replace("_", " ").replace("-", " ").strip()


def read_concepts(spec: Optional[str]) -> List[str]:
    """Read comma-separated, TXT, or JSON list of concepts (lowercased)."""
    if not spec:
        return []
    p = spec.strip()
    if not p:
...
def any_match_target(fn_noext_lower: str, targets_lower: List[str]) -> bool:
    """True if any target concept substring appears in filename."""
    fn = normalize_simple(fn_noext_lower)
    for t in targets_lower:
        if normalize_simple(t) in fn:
            return True
    return False


def match_exact_concept(fn_noext_lower: str, concepts_lower: List[str]) -> Optional[str]:
    """Return the concept whose substring occurs in filename (longest match if multiple)."""
    fn = normalize_simple(fn_noext_lower)
    matches = [c for c in concepts_lower if normalize_simple(c) in fn]
    if not matches:
        return None
    matches.sort(key=len, reverse=True)
    return matches[0]
...
def compute_metrics(
    ...
    canonical_target = target_concepts[0]
    targets_all = target_concepts

    # Load CLIP model
    model, preprocess = clip.load(model_name, device=device, jit=False)
    model.eval()

    # Encode text embeddings (canonical target + each non-target)
    text_prompts = [(canonical_target, f"A photo of {canonical_target}.")]
    for c in non_target_concepts:
        text_prompts.append((c, f"A photo of {c}."))

    tokenized = clip.tokenize([p for _, p in text_prompts]).to(device)
    text_features = model.encode_text(tokenized).float()
    text_features = text_features / text_features.norm(dim=-1, keepdim=True)
    concept2text = {name: text_features[i] for i, (name, _) in enumerate(text_prompts)}

    # Metrics pools
    per_concept_cs: Dict[str, List[float]] = {c: [] for c in targets_all + non_target_concepts}
    targets_pool, nontargets_pool = [], []
    ...
    for p in tqdm(img_paths, desc="Scoring images with CLIP"):
        ...
        # Target?
        if any_match_target(fn_lower, targets_all):
            can_text = concept2text[canonical_target]
            cs = float((img_feat @ can_text.unsqueeze(1)).squeeze().item())
            matches = [t for t in targets_all if normalize_simple(t) in normalize_simple(fn_noext)]
            label = matches[0] if matches else canonical_target
            per_concept_cs[label].append(cs)
            targets_pool.append(cs)
            continue

        # Non-target?
        non_match = match_exact_concept(fn_lower, non_target_concepts)
        if non_match is not None:
            nt_text = concept2text[non_match]
            cs = float((img_feat @ nt_text.unsqueeze(1)).squeeze().item())
            per_concept_cs[non_match].append(cs)
            nontargets_pool.append(cs)
        else:
            skipped += 1
    ...
    if csv_out:
        ...
        with open(csv_out, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["concept", "group", "num_images", "mean_CS", "note"])
            for c in targets_all:
                w.writerow([c, "target", len(per_concept_cs[c]), mean(per_concept_cs[c]),
                            f"CS vs canonical '{canonical_target}'"])
            for c in non_target_concepts:
                w.writerow([c, "non_target", len(per_concept_cs[c]), mean(per_concept_cs[c]),
                            "CS vs own concept"])
            w.writerow([])
            w.writerow(["ESR (1 - mean_CS_targets)", "", len(targets_pool), ESR, f"canonical: {canonical_target}"])
            w.writerow(["PSR (mean_CS_non_targets)", "", len(nontargets_pool), PSR, ""])
            w.writerow(["HM(ESR,PSR)", "", "", HM, ""])
...
def main():
    ap = argparse.ArgumentParser(description="Compute ESR/PSR/HM metrics for CORA or AdaVD results.")
    ap.add_argument("--folder", required=True, help="Folder containing generated images.")
    ap.add_argument("--targets", required=True, help="Target concepts (first is canonical).")
    ap.add_argument("--non_targets", required=True, help="Non-target concepts.")
    ap.add_argument("--model", default="ViT-L/14", help="CLIP model (default: ViT-L/14).")
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--csv_out", default="esr_psr_metrics.csv", help="CSV output file.")
    args = ap.parse_args()
    ...
    print("\n=== Results ===")
    print(f"Canonical target: {res['canonical_target']}")
    print(f"ESR = 1 - mean_CS(targets vs canonical): {res['ESR']['value']:.4f} "
          f"over {res['ESR']['num_images']} images")
    print(f"PSR = mean_CS(non-targets vs own):        {res['PSR']['value']:.4f} "
          f"over {res['PSR']['num_images']} images")
    print(f"HM(ESR, PSR):                             {res['HM']:.4f}")
