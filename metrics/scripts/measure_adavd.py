...
def main():
    ap = argparse.ArgumentParser(description="Unified ESR/PSR/HM runner for AdaVD (original + retain).")
    ...
    ap.add_argument("--base_path", required=True,
                    help="Base results path for this AdaVD run (contains 'original' and 'retain').")
    ...
    retain_folder = os.path.join(args.base_path, "retain")

    # Output CSVs
    orig_csv   = os.path.join(args.base_path, "original_esr_psr_hm.csv")
    retain_csv = os.path.join(args.base_path, "retain_esr_psr_hm.csv")

    # Run both modes
    run_eval(args.esr_script, orig_folder,   targets, args.non_targets, orig_csv,   args.model, args.device)
    run_eval(args.esr_script, retain_folder, targets, args.non_targets, retain_csv, args.model, args.device)
